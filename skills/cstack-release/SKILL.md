...
4. Use `/gstack-landing-report`, `/gstack-plan-eng-review`, and `/gstack-review` for release readiness.
...
6. Run `/gstack-ship` for feature branches and `/gstack-land-and-deploy` only for the approved develop-to-production promotion.
7. Run `/gstack-canary` after deployment, inspect logs and health checks, stop time tracking, and comment the final result with estimated and actual time.
